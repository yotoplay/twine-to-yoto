...
export async function handleAutoGeneratedAudio(
  ...
  if (!apiKey) {
    logger.warn(
      "ElevenLabs API key not available - skipping automatic audio generation",
    );
    return 0;
  }
...
export function clearPlaceholderTracks(yotoJson: YotoJSON): void {
  for (const chapter of yotoJson.content.chapters) {
    chapter.tracks = chapter.tracks.filter(
      (track) => track.trackUrl !== "[placeholder]",
    );
  }
}
...
export function updateTrackUrls(
  ...
  const passage = yotoJson.content.chapters.find(
    (p) => p.key === chapterKeyToFind,
  );
  if (!passage) {
    console.error(
      `Could not find chapter with key "${chapterKeyToFind}" for track "${trackKey}"`,
    );
    return;
  }
  ...
  if (!track) {
    throw new Error(
      `No track found with key "${trackKey}" in chapter "${chapterKeyToFind}"`,
    );
  }
