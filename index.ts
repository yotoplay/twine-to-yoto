#!/usr/bin/env node
import { convertTweeToJson } from './lib/twee2json.js';
import { convertTweeToYoto } from './lib/twee2yoto.js';
...
(async () => {
    ...
    const providedAudioFiles = await handleProvidedAudio(
        input,
        output,
        yotoJson
    );
    ...
    if (argv.upload) {
        try {
            ...
            if (argv.cardid) {
                logger.pending(
                    'Updating existing card in YotoCloud...',
                    argv.cardid
                );
